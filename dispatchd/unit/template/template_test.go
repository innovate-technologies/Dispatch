...
func Test_newFromEtcd(t *testing.T) {
	...
	mockEtcd.EXPECT().Get(gomock.Any(), fmt.Sprintf("/dispatch/templates/%s/%s/%s", Config.Zone, templateName, "name"), gomock.Any()).Return(&etcd.Response{Node: &etcd.Node{Value: "test-temp"}}, nil)
